...
def get_access_token():
    ...
    if not app_id or not app_secret:
        print("错误: 未配置微信凭证。请在 config.json 或 .env 中设置 WECHAT_APP_ID / WECHAT_APP_SECRET")
        sys.exit(1)
...
def find_cover_image(article_dir, cover_arg=None):
    ...
    # 回退到 config.json 中配置的默认封面图
    default_cover = CONFIG.get("cover", {}).get("default_image")
    if default_cover:
        p = Path(default_cover).expanduser()
        if p.exists():
            print(f"  使用默认封面图: {p.name}")
            return p
        else:
            print(f"  警告: 默认封面图不存在: {default_cover}")

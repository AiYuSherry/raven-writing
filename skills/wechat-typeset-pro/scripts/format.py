...
def _render_inline_md(text: str) -> str:
    """对 callout 内容做内联 Markdown 渲染（加粗、斜体、行内代码），
    避免星号裸露在已生成的 HTML <p> 标签里。"""
    # 行内代码：`code`
    ...
    while i < len(lines):
        ...
        if callout_match:
            ...
            while i < len(lines) and lines[i].startswith(">"):
                ...
                i += 1
            content = "\n".join(content_lines)
            # 对内容做内联 Markdown 渲染，避免 **加粗** 星号裸露
            content = _render_inline_md(content)
            # 用特殊标记包裹
            if title:
...
def main():
    ...
    html = md_to_html(content)
    # 去除正文中的 h1 标题（公众号标题需单独填写，不应包含在正文内容里）

...
- **素材库隐私策略**：论文/法律实务素材支持本地导入与检索，但 `data/material_library/files/` 下的原文文件默认作为本地数据忽略，不随仓库上传。
...
- 素材库原文文件、数据库 `data/personal_writing.db`、上传文件 `data/uploads/`、草稿 `草稿/` 和私有配置默认不上传。
...
- [sunyifeisb-art](https://github.com/sunyifeisb-art)：贡献论文素材库功能、文献检索与引用相关能力、Zotero/素材库页面和测试用例。
